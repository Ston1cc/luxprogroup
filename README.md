...
3. **No favicon** — browser tab and bookmarks show a blank/default icon.
4. **No `og:image`** — links shared on WhatsApp/Facebook/Viber show no preview image, which matters for a lead-gen business relying on social shares.
5. **Confirm `luxprogroup.md` is a real, owned, live domain** — canonical URLs and the privacy policy both reference it. If it's not live yet, that blocks launch, not just SEO.
